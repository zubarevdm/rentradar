...
class Settings(BaseSettings):
    ...
    avito_detail_max_per_run: int = 4
    avito_detail_delay_sec: float = 10.0
    # Как часто РЕАЛЬНО дёргать Авито, минут (независимо от общего сбора). У Авито
    # жёсткий рейт-лимит по IP: собираем его реже Циан/Яндекса, чтобы не словить 429.
    avito_collect_interval_min: int = 12
    # Сколько страниц выдачи Авито тянуть за проход (меньше = меньше нагрузка на IP).
    avito_max_pages: int = 2

    # Часы публичного постинга (MSK, через запятую) — пики активности: утро/обед/
    # вечер. 3 слота × max_posts_per_run(1) = 3 поста/день. Исследование LiveDune
    # (26k каналов): >3 постов/день режут охват на 30-40% и гонят отписки; 1-3 —
    # оптимум для не-новостных каналов. Пусто → почасовой режим (poll_interval_min).
    public_post_hours: str = "10,14,20"
    # Разброс времени постинга, сек (±): чтобы посты выходили не ровно в HH:00, а
    # с рандомным сдвигом (не так «палевно», что постит бот). 300 = ±5 мин.
    public_post_jitter_sec: int = 300
    # Запасной интервал постинга, если public_post_hours пуст.
    poll_interval_min: int = 60
    # Окно «свежести» для публичного топа, часов. Дедуп по posted_log не даёт
    # постить один лот дважды между слотами.
    public_lookback_hours: int = 12
    # Частота центрального сбора площадок в минутах (питает канал и личного бота).
    # 5 мин: новые лоты появляются у нас быстро, «в моменте». Чаще смысла мало
    # (площадки не постят так часто) и злее антибот Авито.
    ...
    # Максимум постов за слот — ГЛОБАЛЬНЫЙ топ-N по всем профилям (канал-витрина).
    # 1 на слот × 3 слота = 3 поста/день (см. public_post_hours). Меньше и красивее
    # лучше конвертит, чем много посредственного.
    ...
    # Диспетчер личной рассылки разводит лоты на две «дорожки», чтобы свежие,
    # пойманные в моменте, не стояли в очереди за старым бэклогом:
    # СВЕЖИЕ (появились с прошлой проверки) шлём почти без лимита, кап только
    # страховочный (от флуда при широком фильтре).
    personal_fresh_max: int = 15
    # БЭКЛОГ (был на рынке ещё до фильтра) досылаем мягко, по чуть-чуть за проверку,
    # ВСЕГДА после свежих — так старое не задерживает моментальные.
    personal_backlog_per_check: int = 5
    # Первый прогон нового фильтра: сколько свежайших показать сразу. Остальное
    # уйдёт в бэклог (докапываем постепенно), чтобы не завалить 300 лотами из базы.
    personal_cold_start_limit: int = 12
    ...
    referral_days: int = 7
    # Сколько дней получает пригласивший, когда приглашённый АКТИВИРОВАЛСЯ (подписался
    # на канал и настроил первый поиск). Скромно — действие лёгкое, защита от фарма.
    referral_activation_days: int = 1
    ...
    # Порог «вкусности» (недооценки) для канала. 0 = НЕ требовать сделку: в канал
    # пускаем красивые квартиры независимо от выгодности — лучшие сделки бережём
    # для бота (там персональный поток по фильтрам). Так бот всегда ценнее паблика.
    # Подними, если хочешь, чтобы в канал шли только красивые И выгодные.
    public_min_score: int = 0
