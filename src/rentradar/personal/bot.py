...
def build_router(store: PersonalStore, settings: Settings) -> Router:
    ...
    async def start(message: Message) -> None:
        ...
        await message.answer(
            "👋 Помогу снять квартиру в Москве под ваши критерии.\n\n"
            "Как это работает:\n"
            "🔎 вы задаёте метро, бюджет, комнаты и ремонт\n"
            "📲 слежу за Циан, Яндексом и Авито и присылаю новые подходящие "
            "варианты сюда\n"
            "⚡️ часто раньше, чем их успевают разобрать\n\n"
            f"Первые <b>{settings.free_sends_limit}</b> вариантов бесплатно, дальше "
            "подписка.\n\n"
            "Чтобы начать, нажмите «🔍 Новый поиск» внизу. Настройка займёт минуту.",
